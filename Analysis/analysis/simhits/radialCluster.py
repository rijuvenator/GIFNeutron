...
def setup(self, PARAMS):
	...
	self.F_OUT.cd()
	self.HISTS = {}
	self.HISTS['SUM'] = R.TH1F('h'+'SUM', '', 100, 0, 1000)
	for RING in RINGLIST:
		self.HISTS[RING] = R.TH1F('h'+RING, '', 100, 0, 1000)
		...
		for cham in uniqueChamList:
			...
			for layer in [1,2,3,4,5,6]:
				for cluster in simHitClusters[layer]:
					r = cluster.pos(coordinates='global',units='rphiz')[0]
					#print ch.display('ME{E}{S}/{R}/{C}'), r/100.
					self.HISTS[ch.display('{E}{S}{R}')].Fill(r)
					self.HISTS['SUM'].Fill(r)
...
def makePlots(HISTS):
	for STATION in ['1', '2', '3', '4']:
		...
		canvas.makeLegend()
		canvas.firstPlot.setTitles(X='Distance [cm]', Y='Counts')
		canvas.firstPlot.SetMaximum(10**4)
		canvas.firstPlot.SetMinimum(0.1)
		canvas.scaleMargins(0.8, 'L')
		canvas.finishCanvas()
		canvas.save('pdfs/RadialCluster_ME'+STATION+'.pdf')
		canvas.deleteCanvas()
