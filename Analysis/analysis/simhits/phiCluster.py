...
def setup(self, PARAMS):
	...
	self.F_OUT.cd()
	self.HISTS = {}
	self.HISTS['SUM'] = R.TH1F('h'+'SUM', '', 50, -3.1416, 3.1416)
	for RING in RINGLIST:
		self.HISTS[RING] = R.TH1F('h'+RING, '', 50, -3.1416, 3.1416)
		...
		for cham in uniqueChamList:
			...
			for layer in [1,2,3,4,5,6]:
				for cluster in simHitClusters[layer]:
					p = cluster.pos(coordinates='global',units='rphiz')[1]
					#print ch.display('ME{E}{S}/{R}/{C}'), r/100.
					self.HISTS[ch.display('{E}{S}{R}')].Fill(p)
					self.HISTS['SUM'].Fill(p)
...
def makePlots(HISTS):
	for STATION in ['1', '2', '3', '4']:
		...
		canvas.makeLegend()
		canvas.firstPlot.setTitles(X='Angle w.r.t. +x [rad]', Y='Counts')
		canvas.firstPlot.SetMaximum(10**4)
		canvas.firstPlot.SetMinimum(1)
		canvas.scaleMargins(0.8, 'L')
		canvas.finishCanvas()
		canvas.save('pdfs/PhiCluster_ME'+STATION+'.pdf')
		canvas.deleteCanvas()
