...
for digi in DIGILIST:
	for ring in RINGLIST:
		...
		areaHists[digi][ring] = R.TH1D(digi+'_'+ring,'',lim+1,1,lim+2)
		for idx,idigi in enumerate(range(low,lim+1)):
			if digi=='wire':
				if ring!='11': # non me11
					...
					areaHists[digi][ring].SetBinContent(idigi,a)
				else: # me11
					areaHists[digi][ring].SetBinContent(idigi,me11[idigi]*0.01) # convert me11 dict from mm^2 to cm^2
			else: # comp
				...
				if ring!='11': # non me11
					a = 6*0.5*(cham.t[ring]+cham.b[ring])*cham.h[ring]/lim
					areaHists[digi][ring].SetBinContent(idigi,a)
				else: # me11
					if idigi<130: # me11b
						a = 6*0.5*(47.4+27.4)*107.5/64.
						areaHists[digi][ring].SetBinContent(idigi,a)
					else: # me11a
						a = 6*0.5*(27.4+19.14)*44.5/48.
						areaHists[digi][ring].SetBinContent(idigi,a)
