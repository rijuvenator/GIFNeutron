...
class Canvas(R.TCanvas):
	...
	def drawLines(self):
		self.cd()
		self.LINES = [\
			R.TLine(0, (8. + 0.05)/self.LAYOUT['y'], 1, (8. + 0.05)/self.LAYOUT['y']),
			R.TLine(0, (6. + 0.05)/self.LAYOUT['y'], 1, (6. + 0.05)/self.LAYOUT['y']),
		]
	...
	# save
	def save(self):
		self.SaveAs('patterns_clustered.pdf')

# instantiate canvas
c = Canvas(layout={'x':4, 'y':9})
# pids
labels = [\
	1,                # Lonely
	3, 9,             # 2-Horiz, 2-Vert
