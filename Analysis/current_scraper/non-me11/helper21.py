...
fills = {
		...
		#5395:['11.10.16 08:00:00','11.10.16 13:00:00'], # no data
		5401:['12.10.16 00:00:00','12.10.16 09:30:00'], # formatting issues in output txt file
...
def make_conv(dooffset=False,lumi=True):
	OFFSET = '' if dooffset else 'no_'
	LUMI = 'high_lumi' if lumi else 'all_lumi'
	where = 'data/'+OFFSET+'offset_'+LUMI
	...
	for idx,fill in enumerate(fills.keys()):
		...
		for jdx,columns in enumerate(data):
			...
			if dooffset:
				#print fill,ec,cham,segment,layer
				if float(col[19])>=1.0: continue # skip if max I dev is too big
				if float(col[7])<2.0: continue
				conv[fill][ec][cham][segment][layer] = {
						...
						'slope':float(col[10]),
						'slope_err'  :float(col[11]),
						'maxDev':float(col[19]),
						}
			else:
				if float(col[15])>=1.0: continue # skip if max I dev is too big
				if float(col[7])<2.0: continue
				conv[fill][ec][cham][segment][layer] = {
						'HV':HV,
						'chi2':float(col[6]),
