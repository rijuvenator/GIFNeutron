...
class LumiDBinfo:
    ...
    def getFillTiming(self, fill, verbouse="FALSE"):
        ...
        if(len(rows)>0):
            if(verbouse): kout.printOUT("first BRIL record: "+rows[0][0].strftime('%d.%m.%y %H:%M'))
        cmd = """ select BEGINTIME, ENDTIME, INITLUMI from CMS_RUNTIME_LOGGER.RUNTIME_SUMMARY where LHCFILL="""+str(fill)
        ...
        if(len(rows)>0):
            if(verbouse): 
                for x in rows:
                    kout.printOUT( "RTL: fill begins: "+x[0].strftime('%d.%m.%y %H:%M')
                                        +"; fill ends: "+x[1].strftime('%d.%m.%y %H:%M')
                                        +"; initial limu: "+str(x[2]) )
