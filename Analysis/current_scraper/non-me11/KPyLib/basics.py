...
def getMean(theArray):
    if(len(theArray)==0):
        kout.printERR("basics.getMean : ", "the length of array is 0; returning None")
        return None
    
    sum = 0
    try:
        for entr in theArray : 
            sum+=entr
    except(ValueError, TypeError) : 
        kout.printERR("basics.getMean : ", "the type of array seems to be wrong; returning None")
        return None
    
    return sum/len(theArray)
