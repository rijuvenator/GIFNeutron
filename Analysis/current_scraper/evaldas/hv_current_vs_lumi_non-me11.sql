select    
    change_date,
    dpname,
    max(value) hv_current_nanoamps,
    max(l.instlumi) instantaneous_luminosity
from
  cms_csc_pvss_cond.csc_hv_i_data d join cms_csc_pvss_cond.dp_name2id dp on (dp.id = d.DPID) join CMS_RUNTIME_LOGGER.LUMI_SECTIONS l on (l.starttime < d.change_date and l.stoptime > d.change_date)
where
