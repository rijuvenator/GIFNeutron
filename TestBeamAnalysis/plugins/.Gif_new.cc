...
Gif::Gif(const edm::ParameterSet& iConfig)
{
  ...
  //hits=fs->make<TH1D>("hits","landau 3x3;ADC;N",300,0,1200);
  //strip=fs->make<TH1D>("strips","Occupancy of strips with signals (>13 ADC);Strip;N",112,0.5,112.5);
  //wire=fs->make<TH1D>("wire","digi wires;Wire;N",112,0.5,112.5);
  ...
  //gg2d=fs->make<TProfile>2D("gg2d","2D");
  ...
  //hit_ctime=fs->make<TH1I>("hct","RecHit strip time;Time, ns");
  //hit_atime=fs->make<TH1I>();
...
Gif::endJob() 
{
    ...
    // this is no longer supported with the addition of GIFTree
