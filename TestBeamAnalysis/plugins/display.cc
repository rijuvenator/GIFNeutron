...
void WireStripDisplay(TString address, CSCDetID id, vector<WIRE> &wire, vector<STRIP> &strip, vector<COMPARATOR> &comparator, vector<CSCDetID> &usedChamber, int Run, int Event)
{
    ...
    //this chamber has not been used for eventdisplay
    if (ChamberUsedForEventDisplay(id, usedChamber)) return;
...
void SetSaveNameLegendName(TString& name, TString& legendName, TString address, CSCDetID id, int Run, int Event)
{
    ...
    if (id.Endcap == 1){
        legendName = "ME+" 
            + NumberToString(id.Station) + "/" 
            + NumberToString(id.Ring) + "/" 
            + NumberToString(id.Chamber) + "   " 
            + "Run #" + NumberToString(Run) + "  " 
            + "Event #" + NumberToString(Event);
        name = address + "/MEPlus" 
            + NumberToString(id.Station) + "_" 
            + NumberToString(id.Ring) + "_" 
            + NumberToString(id.Chamber) + "_" 
            + NumberToString(Run) + "_" 
            + NumberToString(Event);
    }
    // Minus Endcap
    if (id.Endcap == 2){
        legendName = "ME-" 
...
{ 
    //in each interesting layer has strip hits
...
vector<vector<double> > StripHitDisplay(CSCDetID id, vector<int>& layer_strip, vector<int>& layer_comparator, vector<STRIP> strip, vector<COMPARATOR> &comparator, double cfeb[])
{
    ...
    for (int i = 0; i < int(layer_strip.size()); i++){
        ...
        //make a vector containing strip hits' layer, since using tgraph, make it 1.5, 2.5...
        vector<double> stripHitsLayer (int(stripHits.size()), tempLayer+0.5);
...
void MakeHit1(Strips &s, vector<double>& stripHits)
{
    //deal with strip singlet
    double sHitPos = -99;
...
void MakeHit2(Strips &s, Strips &sSide, vector<double>& stripHits)
{
    // deal with first last fired strip in one layer
    Strips tempStrip;
    tempStrip.ADCTotal = 0;
    tempStrip.MaxADC = 0;
    //first fired strip in one layer
    if (s.Strip < sSide.Strip){
        //have adjacent fired strip on the right
        if (CheckRight(s, sSide)){
            //good peak
            if (IsGoodPeak(s, tempStrip, sSide)){
    ...
    //last fired strip in one layer
    else if(s.Strip > sSide.Strip){
        //have adjacent fired strip on the left
        if (CheckLeft(s, sSide)){
            // good peak
            if (IsGoodPeak(s, sSide, tempStrip)){
...
void MakeHit3(Strips &s, Strips &sSideL, Strips  &sSideR, vector<double>& stripHits)
{
    ...
    // singlet
    if ( !stripOnTheLeft && !stripOnTheRight){
        MakeHit1(s, stripHits);
    }
    // left, c, no right
    else if( stripOnTheLeft && !stripOnTheRight ){
...
{
    //in each interesting layer has wire hits
    for (int i = 0; i < int(layer_wire.size()); i++){
