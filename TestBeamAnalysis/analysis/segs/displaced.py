						...
						for CHAM in CHAMLIST:
							for lct in lcts:
								...
								for layer in [1, 2, 3, 4, 5, 6]:
									...
									self.HISTS[CHAM][MEAS]['2M'].Fill(fillValD, time)
					f.Close()
					for CHAM in CHAMLIST:
				...
				for MEAS in self.MEASDATA[ATT][0:1]: # only interested in Original for now
					for CHAM in CHAMLIST:
						...
						self.HISTS[CHAM][MEAS] = {\
							...
							'2M' : h2M, #h2M.Rebin2D(BINS, 5, 'hN2M'+str(CHAM)+str(MEAS)),
							'2S' : h2S, #h2S.Rebin2D(BINS, 5, 'hN2S'+str(CHAM)+str(MEAS)),
							'SM' : hSM, #hSM.Rebin2D(50, 50, 'hNSM'+str(CHAM)+str(MEAS)),
							'SS' : hSS, #hSS.Rebin2D(50, 50, 'hNSM'+str(CHAM)+str(MEAS)),
						}
