...
if __name__ == '__main__' and 'submit' in sys.argv:
	...
	outPath = plotsDir+'test_'+str(run)+'.root'
	dryrun = 'dryrun' in sys.argv

	gif_py = open('GifAnalysis.py').read()

	run_fn = open('runFiles/'+str(run)+'.txt').read()
	parent_fn = open('parentFiles/parent_'+str(run)+'.txt').read()
...
process.source.fileNames  = cms.untracked.vstring(%(run_fn)s)
