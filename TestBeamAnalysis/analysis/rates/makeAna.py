...
#where = 'yuiry'
where = 'test'
# track = 'LCT' or 'CLCT' or 'ALCT' or 'comp' or 'wg' or 'segment' or 'strip' or 'rechit'
track = 'CLCT'
...
class MegaStruct():
			...
			for meas in self.FFFMeas[att]:
				...
				for entry in t:
					if track == 'LCT':
						...
						if list(t.lct_id).count(110)>0: nTrack_21 = nTrack_21 + 1
					elif track == 'CLCT':
						if list(t.clct_id).count(1)>0: nTrack_11 = nTrack_11 + 1
						if list(t.clct_id).count(110)>0: nTrack_21 = nTrack_21 + 1
						for clct,clctid in enumerate(t.clct_id):
							if clctid == 1:
								nLayCLCT_11 = nLayCLCT_11 + struct.unpack('1b',t.clct_quality[clct])[0]
								nCLCT_11 = nCLCT_11 + 1
							if clctid == 110:
								nLayCLCT_21 = nLayCLCT_21 + struct.unpack('1b',t.clct_quality[clct])[0]
								nCLCT_21 = nCLCT_21 + 1
					elif track == 'ALCT':
						if list(t.alct_id).count(1)>0: nTrack_11 = nTrack_11 + 1
						if list(t.alct_id).count(110)>0: nTrack_21 = nTrack_21 + 1
					elif track == 'comp':
						if list(t.comp_id).count(1)>0: nTrack_11 = nTrack_11 + 1
						if list(t.comp_id).count(110)>0: nTrack_21 = nTrack_21 + 1
						if doCount:
							nCount_11 = nCount_11 + list(t.comp_id).count(1)
							nCount_21 = nCount_21 + list(t.comp_id).count(110)
					elif track == 'wg':
						if list(t.wire_id).count(1)>0: nTrack_11 = nTrack_11 + 1
						if list(t.wire_id).count(110)>0: nTrack_21 = nTrack_21 + 1
						if doCount:
							nCount_11 = nCount_11 + list(t.wire_id).count(1)
							nCount_21 = nCount_21 + list(t.wire_id).count(110)
					elif track == 'segment':
						if list(t.segment_id).count(1)>0: nTrack_11 = nTrack_11 + 1
						if list(t.segment_id).count(110)>0: nTrack_21 = nTrack_21 + 1
						if doCount:
							nCount_11 = nCount_11 + list(t.segment_id).count(1)
							nCount_21 = nCount_21 + list(t.segment_id).count(110)
					elif track == 'strip':
						if list(t.strip_id).count(1)>0: nTrack_11 = nTrack_11 + 1
						if list(t.strip_id).count(110)>0: nTrack_21 = nTrack_21 + 1
						if doCount:
							nCount_11 = nCount_11 + list(t.strip_id).count(1)
							nCount_21 = nCount_21 + list(t.strip_id).count(110)
					elif track == 'rechit':
						if list(t.rh_id).count(1)>0: nTrack_11 = nTrack_11 + 1
						if list(t.rh_id).count(110)>0: nTrack_21 = nTrack_21 + 1
						if doCount:
							nCount_11 = nCount_11 + list(t.rh_id).count(1)
							nCount_21 = nCount_21 + list(t.rh_id).count(110)
				...
				self.Effs[2][meas] = float(nTrack_21)/float(nTot)
				print '     %s %6i %6i %5i' % ('Eff  ', nTrack_11, nTrack_21, nTot)
				self.nLayCLCT[1][meas] = float(nLayCLCT_11)/float(nCLCT_11)
				self.nLayCLCT[2][meas] = float(nLayCLCT_21)/float(nCLCT_21)
				print '     %s %6i %6i %5i %5i' % ('Count', nLayCLCT_11, nLayCLCT_21, nCLCT_11, nCLCT_21)
				print '     %s %f %f %5i' % ('Count', float(nLayCLCT_11)/nCLCT_11, float(nLayCLCT_21)/nCLCT_21, nTot)
				if doCount:
					self.nCount[1][meas] = float(nCount_11)/float(nTrack_11)
					self.nCount[2][meas] = float(nCount_21)/float(nTrack_21)
					print '     %s %6i %6i %5i' % ('Count', nCount_11, nCount_21, nTot)
	...
	if doNLay:
		def nLay(self, cham, meas):
			return self.nLayCLCT[cham][meas]
		def nLayCLCTVector(self, cham, ff):
			return np.array([self.nLay(cham, self.FFFMeas[att][ff]) for att in self.attVector()])
...
def makePlot(x, y, cham, xtitle, ytitle, title0, title1, pretty=pretty, ylims=[0.0,1.1], loc='bl'):
	...
	plots = []
	if cham==3:
		plots.append(Plotter.Plot(graphs[0], 'Original ME1/1', 'p', 'AP'))
		plots.append(Plotter.Plot(graphs[1], 'Original ME2/1', 'p', 'P'))
	else:
	...
	# Step 2
	if cham==3:
		canvas = Plotter.Canvas('External Trigger', False, 0., 'Internal', 800, 700)
	else:
	...
	if cham==3:
		graphs[0].SetMarkerColor(R.kRed)
		graphs[0].SetMarkerStyle(R.kCircle)
		graphs[0].SetMarkerSize(2.0)
		graphs[1].SetMarkerColor(R.kBlue)
		graphs[1].SetMarkerStyle(R.kSquare)
		graphs[1].SetMarkerSize(2.0)
	else:
		for i,p in enumerate(pretty.keys()):
...
for cham in chamlist:
	...
	if doNLay:
		...
		makePlot(\
				[data.lumiVector(cham, ff) for ff in pretty.keys()],
				[data.nLayCLCTVector(cham, ff) for ff in pretty.keys()],
				cham,
				'Luminosity [Hz/cm^{2}]',
				'<N('+track+' layers)>',
				'NLay'+track,
		...
		'''
makePlot(\
		[[data.lumiVector(1, ff) for ff in pretty.keys()],    [data.lumiVector(2, ff) for ff in pretty.keys()]],
		[[data.nLayCLCTVector(1, ff) for ff in pretty.keys()],[data.nLayCLCTVector(2, ff) for ff in pretty.keys()]],
		3,
		'Luminosity [Hz/cm^{2}]',
		'<N('+track+' layers)>',
		'NLay'+track,
...
makePlot(\
		[[data.currentVector(1, ff) for ff in pretty.keys()], [data.currentVector(2, ff) for ff in pretty.keys()]],
		[[data.nLayCLCTVector(1, ff) for ff in pretty.keys()],[data.nLayCLCTVector(2, ff) for ff in pretty.keys()]],
		3,
		'Mean Current [#muA]',
		'<N('+track+' layers)>',
...
makePlot(\
		[[np.reciprocal(data.attVector()) for ff in pretty.keys()],[np.reciprocal(data.attVector()) for ff in pretty.keys()]],
		[[data.nLayCLCTVector(1, ff) for ff in pretty.keys()],[data.nLayCLCTVector(2, ff) for ff in pretty.keys()]],
		3,
		'Source Intensity 1/A',
		'<N('+track+' layers)>',
