...
nCat = 8
# number of different attenuations
nAtt = 11
...
for i,m in enumerate(measList):
    ...
    # if in the last (nCat-1) measurements by hand set to the measurement type
    # else (nAtts - 1)
    data11[i/nCat if i<len(measList)-nCat+1 else nAtt-1].append(float(nLCT_11)/float(nTot))
    data21[i/nCat if i<len(measList)-nCat+1 else nAtt-1].append(float(nLCT_21)/float(nTot))
